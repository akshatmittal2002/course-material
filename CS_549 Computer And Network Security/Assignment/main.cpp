...
void encrypt(string pt, vector<string> rkb){
	...
	fout << "\t|  " << setfill(' ') << setw(5) << "Round" << "  |  "
		<< setw(21) << "Expansion Permutation" << "  |  "
		<< setw(12) << "Subkey" << "  |  "
		<< setw(8) << "LPT" << "  |  "
		<< setw(8) << "RPT" << "  |" << endl;
	fout << "\t" << setfill('-') << setw(80) << "" << endl;
	fout << "\t|  " << setfill(' ') << setw(5) << "IP" << "  |  "
		<< setw(21) << "-" << "  |  "
		<< setw(12) << "-" << "  |  "
		<< setw(8) << bin2hex(left) << "  |  "
		<< setw(8) << bin2hex(right) << "  |" << endl;
	fout << "\t" << setfill('-') << setw(80) << "" << endl;
	// fout << "\tInitial Permutation -" << endl;
	// fout << "\t\tLPT: " << bin2hex(left) << "\n\t\tRPT: " << bin2hex(right) << endl << endl;
	...
	for (int i = 0; i < 16; i++) {
		...
		fout << "\t|  " << setfill(' ') << setw(5) << i+1 << "  |  "
			<< setw(21) << bin2hex(right_expanded) << "  |  "
			<< setw(12) << bin2hex(rkb[i]) << "  |  "
			<< setw(8) << bin2hex(left) << "  |  "
			<< setw(8) << bin2hex(right) << "  |" << endl;
		fout << "\t" << setfill('-') << setw(80) << "" << endl;

        // fout << "Round " << i + 1 << ":\n"
        //         << "\tExpansion Permuation: " << bin2hex(right_expanded)
        //         << "\n\tSubkey: " << bin2hex(rkb[i])
        //         << "\n\tLPT: " << bin2hex(left)
        //         << "\n\tRPT: " << bin2hex(right) << endl << endl;
	}

	// Swap and Combination
	...
	// fout << "\t|  " << setfill(' ') << setw(5) << "FP" << "  |  "
	// 	<< setw(21) << "-" << "  |  "
	// 	<< setw(12) << "-" << "  |  "
	// 	<< setw(8) << bin2hex(cipher.substr(0,32)) << "  |  "
	// 	<< setw(8) << bin2hex(cipher.substr(8,32)) << "  |" << endl;
    fout << "\t|  Final Cipher Text: " << bin2hex(cipher) << setfill(' ') << setw(42) << "  |" << endl;
	fout << "\t" << setfill('-') << setw(80) << "" << endl;
}

void key_gen(string key, string pt){
    // Hex to binary
	key = hex2bin(key);
...
int main(){
	...
	fout.open("GrpNo_2_CNS_Assignment1.txt");
	// key_gen("0F1571C947D9E859","02468ACEECA86420");

	fout << "CS549: Computer and Network Security\nAssignment 1 - DES\n\n";
	fout << "BY Group 2 -\n" << "\tS1: Akshat Mittal (200101011)\n"
			<< "\tS2: Pradeep Kumar (200101080)\n"
			<< "\tS3: Satvik Tiwari (200101091)\n"
			<< "\tS4: Dheeraj Garg (200101118)\n\n";
	fout << setfill('-') << setw(100) << "-" << endl << endl;

    string pt = s1 + s2;
    string key = s3 + s4;
    fout << "S1: Akshat Mittal (200101011)\n\n";
	fout << "\tPlain Text : " << pt << endl;
	fout << "\tKey        : " << key << endl << endl;
    key_gen(key, pt);
    fout << "\n" << setfill('-') << setw(100) << "-" << endl << endl;

    pt = s2 + s3;
    key = s4 + s1;
    fout << "S2: Pradeep Kumar (200101080)\n\n";
	fout << "\tPlain Text : " << pt << endl;
	fout << "\tKey        : " << key << endl << endl;
    key_gen(key, pt);
    fout << "\n" << setfill('-') << setw(100) << "-" << endl << endl;
    ...
    pt = s3 + s4;
    key = s2 + s1;
    fout << "S3: Satvik Tiwari (200101091)\n\n";
	fout << "\tPlain Text : " << pt << endl;
	fout << "\tKey        : " << key << endl << endl;
    key_gen(key, pt);
    fout << "\n" << setfill('-') << setw(100) << "-" << endl << endl;
    ...
    pt = s4 + s1;
    key = s3 + s2;
    fout << "S4: Dheeraj Garg (200101118)\n\n";
	fout << "\tPlain Text : " << pt << endl;
	fout << "\tKey        : " << key << endl << endl;
    key_gen(key, pt);
}